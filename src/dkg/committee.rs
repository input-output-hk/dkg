...
mod tests {
    ...
    fn valid_phase_2() {
        ...
        let (m1, _broadcast1) =
            DistributedKeyGeneration::<RistrettoPoint>::init(&mut rng, &environment, &mc1, &mc, 0);
        let (_m2, broadcast2) =
            DistributedKeyGeneration::<RistrettoPoint>::init(&mut rng, &environment, &mc2, &mc, 1);
    ...
    fn invalid_phase_2() {
        ...
        let (m1, _broad_1) = DistributedKeyGeneration::<RistrettoPoint>::init(
            &mut rng, &environment, &mc1, &mc, 0,
        );
        let (_m2, broad_2) = DistributedKeyGeneration::<RistrettoPoint>::init(
            &mut rng, &environment, &mc2, &mc, 1,
        );
        let (_m3, broad_3) = DistributedKeyGeneration::<RistrettoPoint>::init(
            &mut rng, &environment, &mc3, &mc, 2,
        );
    ...
    //
    //     let phase_2 = m1.to_phase_2(&environment, &mc1, &fetched_state, &mut rng);
    ...
    //     assert!(broadcast_data.misbehaving_parties[0].2.verify(&mc1.to_public(), &fetched_state[1], &h, 2, 2).is_err());
    // }
}

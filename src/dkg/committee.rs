...
impl<G: PrimeGroupElement> MembersFetchedState1<G> {
    ...
    pub fn from_member_state(member_state: &MemberState1<G>) -> Self {
        Self {
            indexed_shares: member_state.encrypted_shares[0].clone(),
            committed_coeffs: member_state.clone().coeff_comms,
        }
    }
}
...
impl<G: PrimeGroupElement> MemberState2<G> {
    pub fn validate(&self, environment: &Environment<G>) -> Result<Self, DkgError> {
        if self.misbehaving_parties.len() == environment.threshold {
...
mod tests {
    ...
    fn valid_phase_2() {
        ...
        assert!(phase_2.validate(&environment).is_ok());
    }
